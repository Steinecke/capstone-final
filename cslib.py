...
#    if not len(os.listdir(data_dir)) > 0:
    if not 0 < len(os.listdir(data_dir)):
        raise Exception("specified data dir does not contain any files")

#    file_list = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if re.search("\.json", f)]
    file_list: List[Union[bytes, str]] = [os.path.join(data_dir, f) for f in os.listdir(data_dir) if re.search("\.json", f)]
    correct_columns = ['country', 'customer_id', 'day', 'invoice', 'month',
                       'price', 'stream_id', 'times_viewed', 'year']

    ## read data into a temp structure
    all_months = {}
    print("... all month ....",all_months)
...
def fetch_ts(data_dir, clean=False):
    ...
    if clean:
        shutil.rmtree(ts_data_dir)                           #is used to delete an entire directory tree
...
def outlier_correction(df):
    ...
    """
    df['y'] = df['revenue']
    df['y_outlier'] = abs((df.y - df.y.mean()) / df.y.std(ddof=0))
    df.loc[df['y_outlier'] > 2, 'y'] = df['y'].mean()
...
def engineer_features(df, training=True):
    ...
    y - summe aller prices für 30 Tage in die Zukunft
    Einflussfaktoren - unterschiedliche Summen von prices für vorangegangene Tage
    """
    ...
    ## engineer some features, eng_features is ein leeres dict, wenn gefüllt wird es zum dataframe umgewandelt
    eng_features = defaultdict(list)
    previous = [3, 5, 7, 4]  # [7, 14, 21, 28, 35, 42, 49, 56, 63, 70]
