...
@app.route('/dashboard')
def dashboard():
    return render_template('dashboard.html')          # render - ausführen

@app.route('/running', methods=['POST'])
#flask nutzt die HTTP methoden GET (sendet infos vom server) und POST (übernimmt inputs und speichert sie zwischen)um
def running():
    return render_template('running.html')

@app.route('/predict', methods=['GET','POST'])
def predict():
    return model_predict(all,2018,10,20)
    """
...
@app.route('/Py/logs', methods=['GET'])
#def logs(filename):
def logs():
    for filename in model_train():
        print(filename)
    return logs()
    """
